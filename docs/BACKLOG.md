...
> `shared/STATUS.md` 변경로그에 한 줄 남긴다. 최종 갱신: 2026-08-08 (런칭 준비 감사 + 쿠폰 스모크 — 결제 차단 결함 4건 수정).
>
...
### 스모크 진행 상황 (2026-08-08 실측)
결제 직전까지 **전 구간 검증 완료**. 마지막 결제 leg 만 키 대기.

| 단계 | 결과 |
|---|---|
| `/admin/coupons` 쿠폰 생성(SMOKE1000 정액 1,000원) | ✅ |
| `/api/coupons/validate` (정상·소문자 정규화·미존재 코드) | ✅ 1,000원 / COUPON_INVALID |
| 편집기 사진 업로드 → 옵션 선택 → 장바구니 담기 | ✅ |
| 로그인 사용자 DB 카트 동기화 | ✅ (아래 결함 수정 후) |
| 체크아웃 쿠폰 적용 | ✅ 14,800 + 3,000 − 1,000 = **16,800원** |
| `/api/orders` 주문 생성 | ⏸ **503 PAYMENT_UNAVAILABLE** — 전 검증 통과, 키만 대기 |
| Toss 결제창 → confirm → 쿠폰 소비 | ⏸ 키 입력 후 |

**스모크 중 발견·수정한 런칭 차단 결함 2건**(키와 무관, 이미 배포됨):
- **#69** Zod v4 엄격 uuid 가 시드 식별자(`...0010`)를 거부 → `/api/cart`·`/api/orders` 전부 422.
  **시드로 만든 전 상품이 담기·주문 불가**였다. → `uuidLike = z.guid()` 로 통일.
- **#70** `addToCart` 가 동기화 실패를 삼켜 로그인 카트가 조용히 비었다. → 실패 노출.
