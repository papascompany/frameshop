# Phase 2 Security Audit — Order/Payment/Admin Completeness
...
four real, exploitable holes — three of them allow an attacker to either skip
...
### P0-01 — `confirmPayment` does not verify Toss's response amount/orderId/status
...
1. Attacker creates legitimate order `20260513-0001` for `48,000원`.
2. In a parallel tab, attacker creates a second order `20260513-0002` for `100원`.
...
7. **Toss accepts** — the paymentKey/orderId/amount triple is valid (it
   confirmed the 100원 payment). The response has `orderId=20260513-0002,
   totalAmount=100`.
8. We ignore the response and transition `0001` → PAID with paymentKey X.
   Attacker received a 48,000원 frame for 100원.
...
### P0-02 — `/api/render/print` SSRF — service-role fetches any `photo_url` stored in `order_items`
...
CHECK constraint. The ingest path is `createOrder` → `cartItemSchema.photoUrl`
(`src/types/cart.ts:80`) which **does** enforce `httpsUrl()` (per ADR-016).
However:

1. The schema is enforced only at IO boundaries — admins (via service-role) or
...
2. Even within the https constraint, `httpsUrl()` (`src/lib/validation/url.ts`)
   ...
   endpoints — IMDSv2 mitigations don't reach a raw `fetch()`.
...
3. (Or, if/when `cartItemSchema.photoUrl` validation is bypassed by a malformed
   request that the JS client treats leniently — current `httpsUrl()` is regex
   `^https://` only, anything after passes.) `createOrder` snapshots it into
   `order_items.photo_url`.
4. Attacker pays for the order — the render pipeline is fired by `enqueuePrintRender`
...
### P0-03 — `/api/orders` (and `/api/cart`) accept arbitrary `photoUrl` / `previewUrl` from the client; ownership not verified
...
the 300dpi print):**
1. Alice uploads `photo-α` → public URL `https://<sb>/storage/.../α.jpg`.
2. Alice opens her own cart in Bob's browser tab (e.g. shared device);
   browser screenshot or Network panel reveals `photoUrl`.
3. Bob crafts a checkout with `photoUrl = α's URL`. `createOrder` accepts.
4. After payment, the render pipeline fetches α's photo, composites a 300dpi
   print, and stores it under `previews/print/<Bob's orderNo>.png` — which is
   **also a public bucket URL** (`pipeline.ts:230-232`).
5. Bob receives Alice's photo on a frame, paid for at standard price. Without
   Alice's consent.

**Attack scenario B (cart-snapshot URL substitution for SSRF — see P0-02):**
...
1. Add `photoId: ProductId` reference (already present in `cartItemSchema`
   line 76) — require server to look up the photo by `photoId` and replace the
...
### P0-04 — Service-role key compared with non-constant-time string compare
...
1. `a.length !== b.length` short-circuits — an attacker can probe the secret's
...
**Why this is P0, not P1:** `SUPABASE_SERVICE_ROLE_KEY` is *the* master key —
...
## 3. P1 — Fix before next deploy

---

### P1-01 — `previews` bucket is public AND `print_file_url` is enumerable by order_no
...
`order_no` is `YYYYMMDD-NNNN` — sequential daily counter (migration 014). The
filename also includes `item.id` (a UUID) — UUID v4 provides 122 bits of
...
(`previews` per migration 003/004 — `getPublicUrl` returns a public URL), and
...
must be made explicit for `previews/print/*` (300dpi prints) — and the print
...
### P1-02 — Webhook `handleWebhook` ignores Toss event `createdAt` — no replay window
...
### P1-03 — Idempotency race between `/api/payment/confirm` and `/api/webhook/payment`

**File:** `src/lib/payment/confirm.ts:23-79`, `:87-157`

`confirmPayment` transitions `CREATED → PAID`; `handleWebhook` independently
...
3. T2: Confirm route reaches `transitionTo(order.id, 'PAID', ...)`. Now the DB
   status is already PAID — `canTransition('PAID', 'PAID')` returns false-ish
...
4. Between SELECT (line 225) and UPDATE (line 253), if the webhook handler
   ...
   true` — the second overwrites the first mid-flight, and if both writes
...
### P1-04 — `payment_events.status` CHECK constraint is missing
...
```
…which is aspirational — there is no DB-side enforcement. If a future code
...
### P1-05 — No login rate limiting / brute force protection

**File:** `src/app/login/LoginClient.tsx:41-67`

The login form calls `supabase.auth.signInWithPassword` directly from the
browser. Supabase has internal throttling but it's per IP, not per email —
...
### P1-06 — `enqueuePrintRender` fires for every order item on `confirmPayment` and webhook with no failure visibility
...
JSON line — no DB row marking the item as failed, no retry, no admin queue, no
...
Phase-2 docs already mention "queue/Edge Function" — this is the formal P1
ticket.

---

### P1-07 — Unsplash URLs hard-coded in `landing-curation.ts` — third-party CDN dependency in ISR HTML
...
1. Unsplash takedown / hot-link block → broken hero, no fallback. The page
   has no try/catch around `<Image>`.
2. Unsplash CDN response is fetched by next/image's optimiser — adds an
   outbound request to a third-party host on every cache miss.
3. Content compliance — Unsplash license allows commercial use, but specific
...
| P2-02 | `src/lib/upload-ratelimit.ts:19` | `Map<string, Bucket>` grows unbounded — no TTL eviction. On a long-lived Node instance, attacker can fill memory by rotating `sessionId`s. Add periodic prune. |
| P2-03 | `src/lib/security/same-origin.ts:64-67` | Non-production bypass `if (NODE_ENV !== 'production' && !secFetchSite) return true;` — preview deployments run with `NODE_ENV=production` by default on Vercel, so this is fine **only if** preview env confirms it. Worth a deployment-config check. |
| P2-04 | `src/lib/render/print.ts:159-318` | Sharp pipeline allocates ≥4 large buffers (photoOut, clippedPhotoLayer, innerCanvas, innerComposite, withBleed). Memory peak for a 4000×4000 photo on 11×14 print ≈ 200MB. Vercel Pro Node functions cap at 1GB; OK for now, but flag for load test. |
| P2-05 | `src/lib/render/pipeline.ts:243-251` | `console.log` in production path — phase-1 verified 0 `console.log`s; this is a regression. Use `console.warn` or structured logger consistent with rest of codebase. |
...
| P2-07 | `src/lib/payment/confirm.ts:35-37` | "Already PAID → ok: true" is silently idempotent. Caller can't distinguish "you just paid now" from "you already paid 5 minutes ago"; UX-wise the success screen is identical, but for analytics/funnel telemetry it matters. Return a discriminator. |
| P2-08 | `src/app/(shop)/order/lookup/page.tsx` + `src/lib/db/order.ts:290-298` | `findOrderByGuest(orderNo, phone)`: phone is compared with `===`. Acceptable for now, but no rate limit on `/order/lookup` — guest phone enumeration is possible against a known orderNo. Add rate limit + brief lockout. |

## 5. Acknowledged risks / explicit trade-offs

These remain accepted (not findings):
- **ADR-018:** `photos` bucket public — explicit; documented Phase 3 hardening plan.
...
- **No CSRF token (P2-05 phase-1):** mitigated by `isSameOrigin` (cookies
  ...
  `/api/photos/upload`. Render API uses bearer auth instead — correct.
...
| Confirm ↔ webhook race-free | **NO (P1-03)** | YES (Stripe single source of truth via webhook) |
...
| Tax receipt / 세금계산서 | **Not implemented** | Plugin/extension |
| Refund policy disclosure on checkout | Hardcoded in FAQ; not at checkout | YES |
| 통신판매업 신고 | Not present in Footer | YES (Korean ecommerce legal req'd) |

## 7. Recommended remediation order

1. **P0-01** (Toss response verify) — single-file fix, blocks the largest fraud vector.
2. **P0-04** (`timingSafeEqualStr` → `crypto.timingSafeEqual`) — one-line fix.
3. **P0-03** (photo ownership check in `createOrder`) — adds one DB roundtrip,
   eliminates two attack classes (photo theft + SSRF input vector).
4. **P0-02** (URL allowlist for `fetchAsBuffer`) — defense-in-depth on top of P0-03.
5. **P1-03** (confirm↔webhook race) — needs schema migration or RPC; design
...
6. **P1-01** (private `previews` bucket + signed URL) — meaningful schema/code
   churn; pair with ADR-020 like ADR-018.
7. **P1-02** (webhook replay window) — trivial.
8. **P1-04** (CHECK constraint on payment_events.status) — migration only.
9. **P1-05** (login rate limit / MFA) — admin team policy + small route.
10. **P1-06** (render job persistence + retry) — schedule with Phase 2 work
    that already plans Edge Function migration.
11. **P1-07** (host Unsplash mirrors) — design/ops, not urgent security.
12. P2-01 .. P2-08 — schedule in next sprint.
