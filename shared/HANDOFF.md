...
**마이그레이션(5본):** 036/037/040/041/042 — **✅ 2026-08-01 프로덕션 적용 완료**(브라우저 세션,
036→042 통합 실행). 검증 쿼리 21행 전부 일치, 체크아웃 RSC `coupons:true` 런타임 확증(재배포 0).
이력·검증 쿼리 = `docs/MIGRATIONS-APPLY.md` **"2차 적용 완료"** 절.
...
**다음 액션(CTO, 잔여):**
1. ~~마이그 5본 적용~~ — ✅ 2026-08-01 완료(위 마이그레이션 절).
2. **Toss 실키 설정** — 프로덕션 클라키 `test_ck_placeholder` 라 결제 완주 불가(BACKLOG §6, 런칭 전 과제). 설정 후 **쿠폰 소비=confirm 경로 실결제 스모크** 필요.
