...
**Project:** FrameShop
**Started:** 2026-05-12
**Current Phase:** 런칭 준비 — 결제 차단 결함 4건 수정 완료(#66·#67·#69·#70). 쿠폰 스모크는 결제 직전까지 GREEN. **남은 것은 CTO 의 Toss 키 3개 입력 1건.** 상세 = `docs/BACKLOG.md` §0 (2026-08-08)
**Branch:** `main` (FS-X 는 #63 으로 머지·배포됨 — 마이그 001~042 전부 적용)
...
- graceful: 마이그 5본 probe 게이트로 적용 전 무해(UI 비노출, 42703/42P01 노출 금지) — **✅ 2026-08-01 프로덕션 적용 완료**(검증 21행 일치 + 체크아웃 RSC `coupons:true`, 재배포 0) — `docs/MIGRATIONS-APPLY.md` "2차 적용 완료" 절
- [x] 리뷰 → Merge Gate → PR/배포(#63, 2026-07-27 main 라이브) → **마이그 5본 적용(2026-08-01)** — 잔여: 쿠폰 실결제 스모크(Toss 실키 설정 후)
...
- 2026-08-08: [x] **쿠폰 스모크(결제 직전까지 완주) + 런칭 차단 결함 2건 추가 수정(#69·#70)** — 프로덕션 실제 플로우로 진행: 쿠폰 생성(SMOKE1000) → `/api/coupons/validate` 정상(정규화·코드 열거 방지) → 편집기 사진 업로드·옵션 선택 → 장바구니 → 체크아웃 쿠폰 적용 **14,800+3,000−1,000=16,800원** 정확 → `/api/orders` **503 PAYMENT_UNAVAILABLE**(전 검증 통과, 키만 대기·부수효과 0·쿠폰 미소진 확인). 스모크 중 발견한 차단 결함: **(#69)** Zod v4 엄격 `uuid` 가 RFC 버전 비트를 강제해 시드 식별자(`...0010`)를 거부 → `/api/cart`·`/api/orders` 전부 422 → **시드 전 상품 담기·주문 불가**(Toss 키와 무관한 진짜 차단 요인). `uuidLike = z.guid()` 신설 후 식별자 검증 16곳 통일 + 회귀 테스트. **(#70)** `addToCart` 가 동기화 실패를 삼켜 로그인 카트가 조용히 비던 문제 → `CartSyncError` 로 노출. 검증 tsc 0 · eslint 0 · **vitest 819 passed | 14 todo** · build 0. **잔여 = Toss 키 3개 입력(Claude 대행 불가) → 결제창 완주.**
- 2026-08-08: [x] **런칭 준비 감사 + 결제 P0 결함 2건 수정(#66·#67)** — 멀티에이전트 전수 감사(29 에이전트)로 **실결제 100% 실패 결함**을 프로덕션 번들 실측으로 적발: `env-public.ts` 가 `process.env[name]` 동적 접근이라 Turbopack 빌드타임 치환에서 제외 → 브라우저에서 전부 undefined. ① `successUrl/failUrl` 이 `http://localhost:3000` 으로 폴백 → Toss 승인 후 `/payment/success` 미도달 → confirm 미호출 → **카드 승인·캡처 상태로 주문 CREATED 고착** ② `envPublic.supabaseUrl()` throw → `getBrowserSupabase()` 실패 → `isAuthed()` 항상 false → **로그인 카트 DB 동기화 무음 실패**(034/035 로 켠 묶음 동기화가 실제 미동작). 수정: 리터럴 정적 접근 전환(빌드 검증 — 값 인라인·localhost 폴백 소멸) · 복귀 URL `window.location.origin` 우선 · `/api/orders` 503 가드(고아 주문 방지) · webhook 시크릿 미설정 200→503(이벤트 소실 방지). **어드민 설정 승격**: 사업자·법적 고지 탭 6키(company.ts 오버라이드, unstable_cache+updateTag) · 발신 메일(`mail_from`) · 배송 설정 제주/도서산간 추가배송비 컬럼(030 conditional-spread). 프로덕션 검증: 신규 탭·컬럼 렌더 + 배송 저장 "저장되었습니다" 확인. 검증 tsc 0 · eslint 0 · **vitest 812 passed | 14 todo**(798→+14) · build 0. **잔여 = CTO Toss 키 3개 입력 → 쿠폰 실결제 스모크**(BACKLOG §0).
- 2026-08-01: [x] **마이그레이션 2차(036/037/040/041/042) 프로덕션 적용 완료** — CTO 브라우저 로그인(yohan73) + Claude 가 SQL Editor 에서 036→042 통합 실행(비파괴·멱등, "Success. No rows returned"). 검증 쿼리 21행 전부 일치(테이블 6 · cart_projects FK 1 · orders 쿠폰 컬럼 2 · RLS 6 true · 정책 6). 런타임 자동 활성화 확증: 프로덕션 `/checkout` RSC flight `coupons:true`(+기존 points/receipt/surcharge true), 재배포 0. **마이그 001~042 전부 적용 — 세트템플릿·구성규칙·문의·위시·쿠폰 런타임 활성화.** 잔여: 쿠폰 실결제 스모크는 Toss 실키 설정 후(런칭 전 CTO 액션).
